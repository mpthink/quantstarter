    ...
    public void countStrategy(SelectConditions condition) {
        ...
        while (DateUtils.parseUTCTime(end).before(new Date())) {
            ...
            if (ethCandles5ms.size() != 2) {
                ...
                log.info("Finish............" + condition + ",sum: " + sum.stream().mapToDouble(Double::doubleValue).sum());
                //准备写入map中
        ...
        if (flag > 0) {
            for (EthCandles5m candles5m : tempList) {
                if (candles5m.getLow() <= planLossPrice) {
                    sum.add(planLossPrice - buyPrice);
                    //log.info("做多止损,"+time + "," + buyPrice + "," + planLossPrice + "," + (planLossPrice - buyPrice) + "," + (planLossPrice - buyPrice) / buyPrice * 1000);
                    sellflag = false;
                    break;
                }
            }
            if (sellflag) {
                ...
                sum.add(lastClose - buyPrice);
                //log.info("做多按时,"+time + "," + buyPrice + "," + lastClose + "," + (lastClose - buyPrice) + "," + (lastClose - buyPrice) / buyPrice * 1000);
            }
        } else {
            for (EthCandles5m candles5m : tempList) {
                if (candles5m.getHigh() >= planLossPrice) {
                    sum.add(buyPrice - planLossPrice);
            ...
            if (sellflag) {
                Double lastClose = tempList.get(tempList.size() - 1).getClose();
                sum.add(buyPrice - lastClose);
                //log.info("做空按时,"+time + "," + buyPrice + "," + lastClose + "," + (buyPrice - lastClose) + "," + (buyPrice - lastClose) / buyPrice * 1000);
            }
        }
    }
