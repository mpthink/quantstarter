...
import com.think.quantstarter.rest.enums.ContentTypeEnum;
...
import com.think.quantstarter.utils.HmacSHA256Base64Utils;
...
public class APIHttpClient {
    ...
    public OkHttpClient client() {
        ...
        clientBuilder.connectTimeout(okexRestAPIConfig.getConnectTimeout(), TimeUnit.SECONDS);
        clientBuilder.readTimeout(okexRestAPIConfig.getReadTimeout(), TimeUnit.SECONDS);
        clientBuilder.writeTimeout(okexRestAPIConfig.getWriteTimeout(), TimeUnit.SECONDS);
        clientBuilder.retryOnConnectionFailure(okexRestAPIConfig.isRetryOnConnectionFailure());
...
//    ,String simulated
    private Headers headers(final Request request, final String timestamp) {
        ...
        if (StringUtils.isNotEmpty(okexRestAPIConfig.getSecretKey())) {
            //拼接上秘钥，密码，签名和时间戳
            builder.add(HttpHeadersEnum.OK_ACCESS_KEY.header(), okexRestAPIConfig.getApiKey());
            ...
            builder.add(HttpHeadersEnum.OK_ACCESS_PASSPHRASE.header(), okexRestAPIConfig.getPassphrase());
...
//            builder.add(HttpHeadersEnum.x_simulated_trading.header(),simulated);
    ...
    private String getCookie() {
        ...
        cookie.append(APIConstants.LOCALE).append(okexRestAPIConfig.getI18n().i18n());
    ...
    private String sign(final Request request, final String timestamp) {
        ...
        try {
            sign = HmacSHA256Base64Utils.sign(timestamp, this.method(request), this.requestPath(request),
                    this.queryString(request), this.body(request), okexRestAPIConfig.getSecretKey());
    ...
    private String requestPath(final Request request) {
        ...
        url = url.replace(okexRestAPIConfig.getEndpoint(), APIConstants.EMPTY);
        ...
        if(okexRestAPIConfig.getEndpoint().endsWith(APIConstants.SLASH)){
